...
if len(sys.argv) == 6:
    ...
    print "Invalid, please try again! \n(Args expected: [start index for checking] [y's file(\w .h5 subfix)] [#compression ratio(%)] [#repeats/testing to be done] [output folder])"
    sys.exit(0)

TOOLS = "pan,wheel_zoom,box_zoom,reset,save"

def plot_1d(data, lbl, title):
    y = np.array(data)
    index = np.arange(len(data))

    fig, ax = plt.subplots()
    rect = plt.bar(index, y, 0.8,
                 alpha = 0.8,
                 color = 'blue',
                 ecolor = 'black',
                 label = lbl,
                 hatch = "/")
    plt.xlabel('n-th element', horizontalalignment='right')
    plt.ylabel('value')
    plt.title(title, loc='left')
    plt.xticks(index + 0.8, tuple(index))
    plt.show()
...
for i in range(0,repeats):
    ...
    need = 1
    if i < 2:
        plot_1d(y_dct[0:50], 'dct', "DCT before")
    ...
    if i < 2:
        plot_1d(y_dct[0:50], 'dct', "DCT after")
    ...
    file_create(output_folder + "/dct_"+str(i)+".h5", y_cmp, np.shape(y_cmp)[0], np.shape(y_cmp)[1])
    tf = time.time()
...
file_create(output_folder + "/dct_runtime.h5", runtime_list, np.shape(runtime_list)[0], np.shape(runtime_list)[1])
