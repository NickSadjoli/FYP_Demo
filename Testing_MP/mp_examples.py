...
from mp_functions_draw import * #mp_functions contains all the MP algorithms to be tested.
from Phi import Phi
import matplotlib.pyplot as plt
import unittest
import math
...
def SNR_Custom(signal, noise):
    ...
    print Anoise
    return 10 * np.log10(Asignal/Anoise)
...
x_test = [ 7.14872976, 7.99683485, 6.77462035, 7.35682238, 3.46190329, 8.55134831, 1.23216904, 8.02565473, \
...
#get Phi from the Phi.py file
Phi = np.array(Phi)
y_test = np.dot(Phi, x_test)

noise = np.random.normal(0, 1000 * 1/25, y_test.shape[0])
snr_value = SNR_Test(y_test+noise)
...
if chosen_mp == 'omp-scikit':
    ...
    mp_process.fit(Phi, y_test)
    x_mp = mp_process.coef_
else:
    x_mp, _, _= mp_process(Phi, y_test, ncoef=n_nonzero_coefs, verbose=False)
    x_mp_noise, _, _ = mp_process(Phi, y_test+noise, ncoef=n_nonzero_coefs, verbose=False)
'''
...
R_error_noise = Recovery_Error(x_test, x_mp_noise)
print "Recovery Error: ", R_error
print "Recovery Error noise: ", R_error_noise

rms_cur = np.sqrt(np.mean(abs(x_test - x_mp)**2, axis=None))
rms_cur_noise = np.sqrt(np.mean(abs(x_test - x_mp_noise)**2, axis=None))
print "RMS achieved:",rms_cur
print "RMS achieved noise: ", rms_cur_noise
