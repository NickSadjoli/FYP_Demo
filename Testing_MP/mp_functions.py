...
def mp_process(Phi, y, chosen_mp=None, ncoef=None, maxit=1000, tol=1e-5, ztol=1e-12, verbose=False):
    ...
    if chosen_mp == "omp":
        ...
        return cosamp(Phi, y, ncoef, epsilon=tol, max_iter=20, verbose=verbose)
    elif chosen_mp == "bomp":
        if verbose:
            print('\nIteration, relative error, number of non-zeros')
...
def cosamp(phi, y, s, epsilon=1e-10, max_iter=10, verbose=False):
    """
    ...
    for it in range(1,max_iter):
        ...
        #omega_set = np.argsort(P, axis=0, kind="heapsort")[-(2*s):] # large components
