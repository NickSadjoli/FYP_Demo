...
for db in range(1, test_num+1): #cannot start with decibel level of  0
...
'''
plt.plot(index, rms, 'go')
rms_trend = trendline_fit(index, rms)
plt.plot(index, rms_trend(index), 'g--')

plt.ylabel('RMS of {}'.format(chosen_mp))
plt.xlabel('n-th test.') #All tests is with constant sparsity of '+str(s)+)
...
'''
plt.plot(index, rms_w_noise, 'ro')
rms_noise_trend = trendline_fit(index, rms_w_noise)
plt.plot(index, rms_noise_trend(index), 'r--')
...
'''
plt.plot(index, R_error, 'yo')
R_error_trend = trendline_fit(index, R_error)
plt.plot(index, R_error_trend(index), 'y--')
...
'''
plt.plot(index, R_error_w_noise, 'bo')
R_error_noise_trend = trendline_fit(index, R_error_w_noise)
plt.plot(index, R_error_noise_trend(index), 'b--')
